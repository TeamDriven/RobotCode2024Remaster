...
public class SeedPoseEstimation extends Command {
    ...
    public void execute() {
        ...
        LimelightHelpers.SetRobotOrientation(limelightShooter.LIMELIGHT, rot.getDegrees(),
            RobotState.getInstance().fieldVelocity().dtheta, 0, 0, 0, 0);
...
LimelightHelpers.getBotPoseEstimate_wpiBlue_MegaTag2(limelightShooter.LIMELIGHT);
        ...
        if(RobotState.getInstance().fieldVelocity().dtheta > 720) // if our angular velocity is greater than 720 degrees per second, ignore vision updates
        {
...
!(poseEstimate_FrontLL.pose.getY() == 0)) {
                RobotState.getInstance().addVisionObservation(new RobotState.VisionObservation(poseEstimate_FrontLL.pose, poseEstimate_FrontLL.timestampSeconds));
            }
        } catch (Exception e) {
